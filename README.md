...
- **全局快捷键点名**：自定义组合键（修饰键 + 主键，如 `Ctrl+Alt+C`）直接触发抽 1 人；触发范围可选「全局」或「仅 ClassIsland 前台」。默认关闭，在设置里开启。
...
- **四种布局**：完整（主+副按钮）、紧凑（更小）、Mini（只有主按钮）、简约（主按钮 + 文字，隐藏副按钮）。设置里切换。
- **横版 / 竖版**：悬浮窗可横排也可竖排，竖版下四种布局功能一样不少。
- **Call 按钮宽度**：滑块 0–240（0 = 自动），悬浮窗随之变宽。
...
- **抽选时图片**（下拉选内置 + 浏览本地）：6 个内置图标（骰子/名单/星星/对勾/奖杯/随机），也可选本地图片；选了之后**文字不会消失**，图片顶替默认图标、文字保留。
- **结果展示图片**：同样的下拉选项，用在结果窗口名字前面。
- **字体**（下拉）：从系统已装字体里挑。
- **结果字号**（滑块 + 实时数值）：拖滑块时旁边实时显示数字。
- **结果文字色**（色盘 + 开关）：开启用你选的颜色，关掉自动黑白。
- **结果背景色**（色盘 + 开关）：开启用你选的颜色（支持半透明），关掉透明。
- **界面圆角**（滑块）：0–60，作用于悬浮窗按钮、玻璃容器与结果窗口。
- **悬停保持原色**（开关）：开启后光标移到/按住 Call 按钮时保持原本颜色，不做高亮。
...
**插件加载失败，报 `应用程序控制策略已阻止此文件`（错误码 `0x800711C7`）？** 这是 Windows 11「智能应用控制」在拦未签名的第三方插件，与插件本身无关，也没有白名单/排除项。唯一解法是关闭它：**设置 → 隐私和安全性 → Windows 安全中心 → 应用和浏览器控制 → 智能应用控制 → 关闭**（注意：该开关关闭后无法再打开，除非重装 Windows）。

**插件崩溃后被自动禁用，启动时提示"没有加载插件"？** ClassIsland 会在插件目录放一个 `.disabled` 标记来禁用异常插件。去 **应用设置 → 插件** 重新启用；或关掉 **应用设置 → 基本 → 自动禁用异常插件**，避免以后再被自动停用。
